...
#include "MSSQLBPLibrary.generated.h"

USTRUCT(BlueprintType, Category = "MSSQL|Tables")
...
*/
UCLASS()
class UMSSQLBPLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSQL Server")
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSQL Server|Select")
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSQL Server|Select")
		static void SelectDataFromQuery(FString Query, bool &IsSuccess, FString &ErrorMessage, TArray<FMSSQLDataTable>
			&ResultByColumn, TArray<FMSSQLDataRow> &ResultByRow);
	...
	*/
	...
	/**
	* Asynchronously executes select query to the database to select an Image
	*
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSql Server|Select")
		static void GetSelectedTable(TArray<FMSSQLDataTable>
			&ResultByColumn, TArray<FMSSQLDataRow> &ResultByRow);

	/**
	* Gets the Selected Image from the database after the asynchronous select query has been successfully executed
	*
	* @param	ResultByColumn	Returns the result of the Selected Query as an array of SQLDataTable structure
	* @param	ResultByRow  	Returns the result of the Selected Query as an array of SQLDataRow structure
	*/
	...
	* @param	ErrorMessage	Returns the exception message thrown if the Query execution fails
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSQL Server|Update")
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSQL Server|Update")
		...
		/**
	* Asynchronously Updates image to the database from the hard drive
	*
	* @param	Query           Update Query which inserts or updates image to the database
	* @param	UpdateParameter Value which has been passed as a parameter for updating the image in the previous input, 
                               without the @ symbol
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "MSSQL Server|Update")
		static void UpdateImageFromQueryAsync(FString Query, FString UpdateParameter, FString ImagePath);
